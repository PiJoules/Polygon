...
Physics
    ...
    The player moves around the screen by tilting the phone. The x and y components of the acceleration are then integrated twice to get the position. A viscous friction force opposes the player's motion with a force proportional to velocity squared. This prevents the player from moving too fast but does not oppose motion at low speeds. Collision physics between the player and the wall are also included. When the oval hits the side of the phone, its velocity in that direction is reversed and scaled by a restitution factor.

Calibration

    The phone can be "calibrated" so that the phone can be held at a different zero angle at which the player will not move. This is done by saving the acceleration values at the new zero angle and adding these values to the raw accelerometer readings.

Scoring

    The five highest scores the player gets by playing are saved loclly onto a text file on the phone. These scores are associated with the name the player chooses to save his score as. These scores are also saved globally on a databse through an online server that parses the names and scores on the text file. Unlike the local scores though where the same player can hold mutliple scores, the global scores only record the highest score achieved by one user. This way, the same player will not hold multiple places in the global high scores.
